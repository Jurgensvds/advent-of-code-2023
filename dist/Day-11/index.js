...
let GALAXY_MAP = [];
let ALL_SHORTEST_PATHS = new Map();
let GALAXY_EMPTY_SPACES = { cols: [], rows: [] };
let ALL_PATHS = [];
function main(filePath = 'src/Day-11/input.txt') {
...
exports.main = main;
// Part One - START
function partOne(multiplier, partOne = true) {
    const allGalaxyCoords = getAllGalaxyCoordsOnMap(GALAXY_MAP);
    ALL_PATHS = [];
...
// Part Two - END
// Utility Functions
function mapInputToGalaxyMap(input) {
    GALAXY_MAP = input.trim().split('\n').map((row) => row.trim().split(''));
}
function findGalaxyEmptySpaces(map) {
...
function getAllGalaxyCoordsOnMap(map) {
    const allGalaxyCoords = [];
    for (let row = 0; row < map.length; row++) {
        for (let col = 0; col < map[0].length; col++) {
            if (map[row][col] === '#') {
...
function shortestPathBetweenTwoGalaxies(start, end) {
    let currentLocation = (0, lodash_1.cloneDeep)(start);
    const path = [];
    let moveX = true;
    while (currentLocation.x !== end.x || currentLocation.y !== end.y) {
        ...
        moveX = !moveX;
        path.push((0, lodash_1.cloneDeep)(currentLocation));
    }
    return path;
}
function getEmptySpaceMoves(path) {
